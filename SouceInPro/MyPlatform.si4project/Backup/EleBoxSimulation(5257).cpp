...
bool EleBoxSimulation::syncBuf(void)
{
	/* 申请内存空间 */
	...
	{
		printf("Read register error in syncBuf\n");
...
bool EleBoxSimulation::handleSTBuf(uint16_t* pBuf)
{
...
bool EleBoxSimulation::electrify48vOut(void)
{
	/* 设置寄存器 */
	int nRegVal = (48 / 0.02563);
	
	/* 设置寄存器 */
	if(writeHoldingRegister(en_V4_Rbt48vVol, nRegVal) < 0)
	{
		return false;
	}
...
bool EleBoxSimulation::blackout48vOut(void)
{
	/* 设置寄存器 */
	if(writeHoldingRegister(en_V4_Rbt48vVol, 0) < 0)
	{
		return -1;
	}
...
int EleBoxSimulation::resetEStop(void)
{
	if(writeHoldingRegister(en_V4_EStopFlg, 0) < 0)
	{
		return -1;
	}
...
int EleBoxSimulation::resetSafetyGuradStop(void)
{
	return 0;
}

int EleBoxSimulation::setAnalogOutputMode(const int& nIndex, const int& nMode)
{
	int nRegisterAddr = 0;
	...
	switch(nIndex)
	{
		case en_AnalogIO_CH1:
			nRegisterAddr = en_V4_RetCh1AnalogOutMode;		
			break;

		case en_AnalogIO_CH2:
			nRegisterAddr = en_V4_RetCh2AnalogOutMode;
			break;
...
int EleBoxSimulation::setAnalogOutputValue(const int& nIndex, const double& dVal)
{
	int nRegisterAddr = 0;
...
int EleBoxSimulation::setDO(const int& nIndex,const int& nVal)
{
	/* 获取当前的IO状态 */
	uint16_t pTmpBuf = 0;
	...
	/* 写寄存器 */
	if(writeHoldingRegister(en_V4_RetDO, pTmpBuf) < 0)
	{
		return -1;
	}
...
int EleBoxSimulation::setCO(const int& nIndex,const int& nVal)
{
	uint16_t pTmpBuf = 0;
	...
	/* 写寄存器 */
	if(writeHoldingRegister(en_V4_RetCO, pTmpBuf) < 0)
	{
		return -1;
	}
